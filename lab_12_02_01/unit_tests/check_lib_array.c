...
extern key_t key;
extern mysort_t mysort;
extern compar_t compare_int;
...
// Указатель на указатель конца выходного массива NULL на вход фильтрации
START_TEST(key_null_dst_end)
{
    ...
    rc = key(pbs, pes, pbd, NULL);
    ck_assert_int_eq(rc, ERR_NULLPTR);
}
END_TEST

// Указатель на начало выходного массива NULL на вход фильтрации
...
START_TEST(key_one_elem)
{
    ...
    int *pbs = arr;
    int *pes = arr + 1;
    int *pbd = malloc(sizeof(arr));
    int *ped;
    int rc;

    ck_assert_ptr_nonnull(pbd);

...
if [[ "$4" != "-m" ]]; then
    if ! ./app.exe "${args[@]}" < "$1" > "./out/out_stream.tmp"; then
        exit 1
    fi
else
    if ! valgrind -q --error-exitcode=1 --leak-check=full --show-leak-kinds=all ./app.exe "${args[@]}" < "$1" > "./out/out_stream.tmp"; then
        exit 1
    fi
fi

# Проверка компаратором вывода программы и целевого вывода
if ! ./func_tests/scripts/comparator.sh "./out/out_stream.tmp" "$2"
then
    exit 1
fi
exit 0
